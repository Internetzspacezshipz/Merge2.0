...
public class Movement : BaseUnit
{
	...
	void Update ()
    {
        ...
        //{
            Move(horizontalInput * sprintBonus);
        //}
    ...
    {
        if (other.transform.CompareTag("EnemyTag") || other.transform.CompareTag("KillZoneTag"))
        {
            Die();
        }
    }
    ...
    //This kills the CPU
    IEnumerator OverloadCPU()
    {
        while (true)
        {
            for (int i = 0; i < 100000; i++)
            {
                GameObject tmp = new GameObject(name = "Blah");
                tmp.isStatic = false;
                tmp.SetActive(false);
            }
        }
